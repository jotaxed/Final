        ...
        for (Entry entry : parameters.entrySet()) {
            ...
            if (field2 != null) {
                try {
                    ...
                    gr.W("Server option \"" + ((String) entry.getKey()) + "\" could not be set: Illegal Access");
                } catch (IllegalArgumentException e2) {
                    gr.W("Server option \"" + ((String) entry.getKey()) + "\" could not be set: Bad Type");
                }
            } else {
                gr.S("Unexpected server option: " + ((String) entry.getKey()) + " = \"" + ((String) entry.getValue()) + "\"");
            }
        }
        StringBuilder stringBuilder = new StringBuilder();
        for (Field field22 : hashMap.values()) {
            if (((Parameter) field22.getAnnotation(Parameter.class)).required()) {
                gr.W("Required server option missing: " + ((Parameter) field22.getAnnotation(Parameter.class)).name());
